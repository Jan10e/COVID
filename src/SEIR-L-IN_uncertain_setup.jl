...
# mutable indicates that the fields can change. Standard is immutable
...
function seir_model!(dx, x, pars, t) # exclamation mark means that function modifies its arguments
    ...
    dx[inds[1]] = dS = [-1 / (N * seir.t_infectious) *
                        sum(seir.R0[ind[1]] * S[ind[2]] * I[ind[3]] *
                            T4.get([
        ind[1] - 1,
        ind[2] - 1,
        ind[3] - 1,
        k - 1,
    ]) / T2.get([k - 1, k - 1]) for ind in inds_S) for k = 1:L]
    ...
    dx[inds[5]] = dLAG = 1 / seir.t_latent *
                         [sum(int_care.percentage[ind[1]] * E[ind[2]] *
                              T3.get([ind[1] - 1, ind[2] - 1, k - 1]) /
                              T2.get([k - 1, k - 1]) for ind in inds_LAG) for k = 1:L] -
                         1 / int_care.t_lag * LAG
...
function plot_results(
    ...
    for i = 1:6
        ...
        if save
            savefig("figures/seir-icu-$i.pdf")
        end
    end
end
